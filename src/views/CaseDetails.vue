        ...
        <CRow class="page-heading" style="padding-left: 15px; padding-right: 15px">
          <CCol
            ><h1><CIcon
                      ...
                      class="escalated"
                      size="xl"
                    />&nbsp;<CButton size="sm" :color="getSeverityColor(case_data.severity)">{{
                      getSeverityText(case_data.severity)
                    }}</CButton>&nbsp;{{ case_data.title }}</h1></CCol
          >
          <CCol
                col="6"
                @mouseenter="edit_tags_hint = true"
                @mouseleave="edit_tags_hint = false"
              >
                <span v-if="!edit_tags"
                  ><TagList :tags="case_data.tags" label="name" /><a
                    v-if="edit_tags_hint"
                    @click="editTags()"
                    ><CIcon style="cursor: pointer" name="cilPencil" size="sm" /></a
                ></span>
                <span v-if="edit_tags">
                  <multiselect
                    ...
                    style="z-index: 50"
                  ></multiselect>
                  <CButton color="danger" @click="edit_tags = !edit_tags" size="sm"
                    ><CIcon name="cilXCircle"
                  /></CButton>
                  <CButton color="primary" @click="saveTags()" size="sm"
                    ><CIcon name="cilSave"
                  /></CButton>
                </span>
              </CCol>
          ...
          <CTab title="Case Details" active>
            <CCardBody style="padding-top: 5px">
              ...
              <CRow>
                ...
                ><CCard>
                <CCardBody @mouseover="edit_description_hint = true"
                  ...
                  <span v-if="edit_description">
                    <CButton color="danger" @click="edit_description = false" size="sm"
                      ><CIcon name="cilXCircle" /></CButton
                    >&nbsp;<CButton color="primary" @click="saveDescription()" size="sm"
                    ...
                    <markdown-editor
                      ...
                      @change="case_data.description = $event"
                    ></markdown-editor
                    ><br />
                ...
                <CCol col=3>
                  ...
                  <span
                    ...
                    "
                    ><b>Applied Case Template: </b>
                    {{ case_data.case_template.title }}</span
                  >
                  </CCardBody>
                  </CCard>
                </CCol>
                    ...
                    <template #events="{ item }">
                      <td>
                        <CButton
                          ...
                          "
                          v-if="!(filteredBySignature() || item.related_events_count < 1)"
                          color="dark"
                          size="sm"
            ...
            <div v-else>
              <CRow style="padding: 10px">
                <CCol>
                  <CButton
                    color="primary"
                    @click="caseTaskModal = !caseTaskModal"
                    size="sm"
                    >Add Task</CButton
                  >
                </CCol>
    ...
    removeCaseTemplate() {
      ...
      this.$store.dispatch("updateCase", { uuid, data }).then((resp) => {
        this.$store.commit("show_alert", {
          message: "Successfully removed the case template",
          type: "success",
        });
        this.loadData();
        this.reloadTasks = Math.random();
