<template>
  <CCard>
    <CCardBody
      ><CRow>
        <CCol col="6">
          ...
          <span v-else
            ...
            >
            closed the case with the following comment at
          ...
          <CButtonGroup>
            <CButton
              ...
              v-if="
                ...
                settings.allow_comment_editing &&
            ...
            <CButton
              ...
              v-if="
                ...
                !comment.is_closure_comment &&
                current_user.permissions['delete_case_comment']
              "
      ...
      <vue-markdown v-if="!edit">{{ comment.message }}</vue-markdown
      ><span v-else>
        <markdown-editor
          height="auto"
          theme="info"
          :value="comment.message"
          @change="editted_comment = $event"
        ></markdown-editor>
        <!--<CTextarea :value="comment.message" @change="editted_comment = $event" rows="5"></CTextarea>-->
        <CButton color="danger" @click="edit = !edit" size="sm"
          ><CIcon name="cilXCircle"
        /></CButton>
        <CButton color="primary" @click="updateComment()" size="sm"
...
export default {
  ...
  methods: {
    updateComment() {
      ...
      this.$store
        .dispatch("updateComment", { uuid, data: { message: this.editted_comment } })
        .then((resp) => {
