...
<div><link rel="stylesheet" href="https://unpkg.com/vue-multiselect@2.1.0/dist/vue-multiselect.min.css">
    <CModal title="Edit Case Template" :centered="true" size="xl" :show.sync="modalStatus">
    <div name="create-case-template-step-1" v-if="step == 1">
        <CForm @submit.prevent="createCase" >
            ...
            <div role="group" class="form-group">
                ...
                <multiselect v-model="template.tags" placeholder="Select tags to apply to this input" :taggable="true" tag-placeholder="Add new tag" track-by="name" label="name" :options="tag_list" :multiple="true" @tag="addTag" :close-on-select="false">
                </multiselect>
            </div>
        </CForm>
      </div>
      <div name="create-case-template-step-2" v-if="step == 2"  style="overflow-y: scroll; max-height: 500px">
          ...
          <draggable v-model="tasks" group="tasks" @start="drag=true" @end="drag=false">
                ...
                <CListGroupItem v-for="task in template.tasks" :key="task.order">
                <div v-if="!task.saved">
                    <CRow>
                        <CCol col="1">
      ...
      <div name="create-case-template-step-3" v-if="step == final_step">
          ...
          <br><b>Tasks</b><br><br>
          <CListGroup>
              <CListGroupItem v-for="task in template.tasks" :key="task.id">
          ...
          <br><br>
          <b>Default Tags</b><br>
          <li style="display: inline; margin-right: 2px;" v-for="tag in template.tags" :key="tag.name">
      ...
      <template #footer>
          ...
          <CButton v-if="step == final_step" @click="updateCaseTemplate()" color="primary">Update</CButton>
      </template>
    </CModal>
</div>
</template>
